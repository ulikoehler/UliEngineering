...
class TestFFT(unittest.TestCase):
    def testBasicFFT(self):
        ...
        self.assertEqual(fft.frequencies.shape[0], rand.shape[0] / 2)
        self.assertEqual(fft.amplitudes.shape[0], rand.shape[0] / 2)
        self.assertEqual(fft.frequencies.shape, fft.amplitudes.shape)
    ...
    def testCutFFTDCArtifactsMulti(self):
        ...
        # Insert artificial peak
        self.assertEqual(fft_cut_dc_artifacts_multi(x, [y1], return_idx=True), 10)
        self.assertEqual(fft_cut_dc_artifacts_multi(x, [y1, y2], return_idx=True), 15)
    ...
    def testParallelFFTReduce(self, name, removeDC):
        ...
        if removeDC:
            self.assertEqual(fft.frequencies.shape[0], fft.amplitudes.shape[0])
        else:  # With DC
            self.assertEqual(fft.frequencies.shape[0], 50)
            self.assertEqual(fft.amplitudes.shape[0], 50)
        self.assertEqual(fft.frequencies.shape, fft.amplitudes.shape)
...
class TestClosestFrequency(unittest.TestCase):
    ...
    def test_find_closest_value_noangle(self):
        fftx = np.asarray([1,2,3,4,5])
        fft = FFT(fftx, fftx * 2)
        self.assertEqual((1, 2, None), fft.closest_value(0.))
