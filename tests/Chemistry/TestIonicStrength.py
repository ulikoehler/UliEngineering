...
class TestIonicStrength(unittest.TestCase):
    def test_nacl(self):
        ...
        result = ionic_strength([0.1, 0.1], [1, -1])
        assert_approx_equal(result, 0.1, significant=5)

    def test_cacl2(self):
        # CaCl2 0.1 M: Ca2+ 0.1M, Cl- 0.2M
        # I = 0.5*(0.1*4 + 0.2*1) = 0.5*(0.4+0.2) = 0.3
        result = ionic_strength([0.1, 0.2], [2, -1])
        assert_approx_equal(result, 0.3, significant=5)

    def test_mgso4(self):
        # MgSO4 0.1 M: Mg2+ 0.1M, SO4^2- 0.1M
        # I = 0.5*(0.1*4 + 0.1*4) = 0.4
        result = ionic_strength([0.1, 0.1], [2, -2])
        assert_approx_equal(result, 0.4, significant=5)

    def test_zero(self):
        result = ionic_strength([0, 0], [1, -1])
        assert_approx_equal(result, 0.0, significant=5)
    ...
    def test_mixed_electrolyte(self):
        ...
        result = ionic_strength([0.1, 0.05, 0.2], [1, 2, -1])
        assert_approx_equal(result, 0.25, significant=5)


class TestIonicStrengthFromPairs(unittest.TestCase):
    def test_nacl(self):
        result = ionic_strength_from_pairs([(0.1, 1), (0.1, -1)])
        assert_approx_equal(result, 0.1, significant=5)

    def test_cacl2(self):
        result = ionic_strength_from_pairs([(0.1, 2), (0.2, -1)])
        assert_approx_equal(result, 0.3, significant=5)
