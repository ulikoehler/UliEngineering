...
class IntInterval(__Interval):
    ...
    def __add__(self, i):
        """Add an integer to the interval."""
        return self.__radd__(i)

    def __rsub__(self, i):
        """Subtract the interval from an integer (reverse subtract)."""
        if not isinstance(i, numbers.Integral):
    ...
    def __call__(self, *args):
        """Slice the interval over the provided arrays."""
        if not args:
    ...
    def __mul__(self, n):
        """Multiply the interval by a number."""
...
class GeneratorCounter:
    ...
    def __len__(self):
        """Return the count of items iterated so far."""
        return self.count
