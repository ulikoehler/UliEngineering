...
class ListIterator:
    ...
    Equivalent to (v for v in lst) except calling len() reveals how many
    values are left to iterate.
...
class PeekableIteratorWrapper:
    ...
    The buffer is managed in a stack-like manner (LIFO) so you can un-get
    multiple values.
    """

    def __init__(self, child):
        """Initialize a PeekableIteratorWrapper with a given child iterator."""
        self.buffer = []
        self.child = child
    ...
    def __len__(self):
        """Return len(child). Only supported if child support len()."""
