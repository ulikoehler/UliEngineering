...
class HaleQuerryAbsorptionModel:
    ...
    def __call__(self, wavelength):
        """
        Interpolate the extinction coefficient for the given wavelength (in μm).
        ...
        if np.any(np.less(wavelength, min_wl)) or np.any(np.greater(wavelength, max_wl)):
            raise ValueError(f"Wavelength {wavelength} μm is out of bounds ({min_wl}–{max_wl} μm) for Hale-Querry model.")
